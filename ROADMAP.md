...
| Testes automatizados | ✅ `vitest` + 88 testes unitários em 9 ficheiros (permissões, formatação, juros, extrato, contas financeiras, etc.) + teste de integração real de isolamento multi-tenant (`pnpm test:db`, contra a BD Neon do utilizador); faltam testes de autorização e2e via HTTP |
...
7c. ✅ Testes automatizados — `vitest`, criado 2026-07-06 com testes unitários de permissões e o teste de integração de isolamento multi-tenant (item 12) contra a BD Neon real do utilizador. **Em 2026-07-25 são 88 testes unitários em 9 ficheiros** (ver a tabela de maturidade).
