...
| CRUD de ocorrências | 🟡 Parcial — fotos anexadas resolvidas 2026-07-09 (`ocorrencia.fotoUrl`, via Vercel Blob); atribuição a um fornecedor resolvida 2026-07-26 (`ocorrencia.fornecedorId`, seletor visível ao admin, testado em runtime); falta o fluxo de aprovação de despesas (ver Fase 4, item 3) |
...
| Testes automatizados | ✅ `vitest` + 98 testes unitários em 10 ficheiros (permissões, formatação, juros, extrato, contas financeiras, remapeamento de importação, etc.) + 13 ficheiros de integração real contra a BD Neon do utilizador (`pnpm test:db`, 47 testes, incl. isolamento multi-tenant, round-trip de exportação/importação de condomínio e controlo de acesso por subscrição); faltam testes de autorização e2e via HTTP |
...
4. Mensagens internas. ~~Confirmação de leitura de convocatórias/avisos~~ — **feito 2026-07-26**, ver linha "Assembleias/atas" e "Comunicação" na tabela de maturidade.
...
7c. ✅ Testes automatizados — `vitest`, criado 2026-07-06 com testes unitários de permissões e o teste de integração de isolamento multi-tenant (item 12) contra a BD Neon real do utilizador. **Em 2026-07-26 são 98 testes unitários em 10 ficheiros** (ver a tabela de maturidade).
