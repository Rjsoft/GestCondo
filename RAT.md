...
| Titulares | Condóminos (proprietários e inquilinos), administradores e fornecedores com conta na aplicação, **e também pessoas sem conta** cujos dados de contacto o administrador registe: o proprietário de uma fração (`fracao.contactoEmail`/`fracao.contactoTelefone`, campos que existem desde o início) e titulares adicionais de uma fração (`fracao_titular`, cujos contactos passaram a ter formulário na interface em 2026-08-24). **Correção 2026-08-24**: a redação anterior dizia apenas "com conta na aplicação", o que já era impreciso antes desta data. A aplicação **não envia comunicações** a estas pessoas — avisos e convocatórias saem só para membros com conta aprovada. **Por analisar**: base legal e prazo de conservação específicos para titulares sem conta. |
...
**Acesso convidado a uma ata (achado F13, `docs/audit/USABILITY_FINDINGS.md`, 2026-07-31)**: o administrador pode gerar um link público, sem autenticação, que dá acesso de leitura a uma única ata já aprovada (sem anexos) a um destinatário externo à sua escolha (ex. advogado, comprador de fração), por um prazo definido por si (máx. 90 dias), revogável a qualquer momento. **Destinatários**: alarga-se aqui, pela primeira vez nesta finalidade, a **qualquer pessoa com posse do link** — o controlo de quem recebe o link é inteiramente do administrador, fora da aplicação (ex. email, mensagem), a mesma lógica de responsabilidade já aplicada ao `codigoConvite` do condomínio. **Base legal**: consentimento/decisão do responsável pelo tratamento em cada partilha concreta, dentro do interesse legítimo de prestar contas e cumprir obrigações de transparência para com terceiros com interesse legítimo próprio (ex. potencial comprador). **Categorias de dados**: as mesmas da ata (nome do representante presente/procurador, permilagem votante, sentido de voto, texto da ata) — não inclui dados financeiros individuais nem contactos. **Prazo de conservação**: o registo do link (`acesso_convidado`) mantém-se mesmo depois de expirado/revogado, como histórico de a quem foi dado acesso — sem prazo de eliminação automática definido, revisto no mesmo ciclo dos restantes registos de auditoria.
