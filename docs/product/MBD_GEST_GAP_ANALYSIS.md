...
|---|---|---|---|---|---|---|---|---|---|---|
| G01 | Exercício | Exercício contabilístico como entidade (saldo inicial/final, fecho, reabertura auditada) | Pág. 3, 7, 10 ("Saldo inicial em 01-01-2025") | 🟡 implementado em desenvolvimento (Fase A.1, 2026-07-24), pendente produção | `exercicioFinanceiro` (`lib/db/schema.ts`), `app/actions/exercicios.ts` — fecho/reabertura auditados, exclusão de sobreposição de datas | Não — resolvido em dev | Médio — sem produção validada, ainda depende de nova migração/verificação antes de considerar concluído | P1.1 | — | Aplicar migração e validar em produção |
| G02 | Contas | Conta bancária como entidade (IBAN, banco, tipo à ordem/prazo/caixa, saldo próprio) | Pág. 3 ("BCP ... ORD", "BCP ... PRZ"), pág. 8 (saldo por conta mês a mês) | 🟡 implementado em desenvolvimento (Fase A.1, 2026-07-24), pendente produção | `contaFinanceira` (`lib/db/schema.ts`), `app/actions/contas-financeiras.ts` — IBAN validado/mascarado, associação em massa de movimentos antigos por `destino` | Não — resolvido em dev | Baixo — a segregação formal por conta já existe; falta produção | P1.1 | G01 | Aplicar migração e validar em produção |
...
4. **`docs/audit/SYSTEM_DATA_MAP.md`** (bucket Vercel Blob "público") ficou desatualizado dentro do próprio dia 2026-07-22 face a `SECURITY_AUDIT.md`/`FUNCTIONAL_GAPS.md` (bucket privado). **Não corrigido nesta sessão** — fora do âmbito direto desta auditoria financeira; sinalizado aqui para correção numa próxima sessão focada em `docs/audit/`.
