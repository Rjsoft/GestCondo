...
| G04 | Receitas | Adiantamentos de condómino como conceito próprio (distinto de "pago") | Pág. 3 ("Pagamentos adiantados", "Adiantamentos recebidos"), pág. 10 (Passivo: "Adiantamento de condóminos") | ✅ **corrigido 2026-08-24** | Não encontrado — só `movimento.pago: boolean` | Sim | Baixo — hoje um adiantamento teria de ser lançado como uma quota futura marcada paga antecipadamente, o que funciona mas não é visível como "crédito disponível" | P2 | G01 | Ver secção 7 — pode ser um saldo calculado, não necessariamente nova tabela | <br />**Correção 2026-08-24**: esta linha dava os adiantamentos como não implementados. É falso — existe a tabela `creditoFracao` (livro-razão do crédito por fração, com movimentos de tipo "adiantamento" e "aplicacao"), o separador "Adiantamentos" em `/financas` e diálogos próprios para registar, aplicar e devolver crédito. O que **continua** por fazer é o que o `ROADMAP.md` já regista à parte: o balanço patrimonial não inclui os adiantamentos como passivo próprio.
| G05 | Receitas | Quotas extraordinárias distintas, ligadas a uma deliberação de assembleia | Pág. 3 ("Quota Extraordinária Obras", "Quota Extra - Rep. Fachadas") | ✅ em produção desde 2026-07-25 (Fase B) | `movimento.assembleiaPontoId` (FK simples, `lib/db/schema.ts`), `getPontosAprovadosParaQuota`/validação em `criarMovimento`/`atualizarMovimento` (`app/actions/financas.ts`) — só pontos aprovados de assembleias com ata aprovada são elegíveis | Não | Baixo — migração puramente aditiva (`drizzle/0027_slippery_reavers.sql`), aplicada em produção 2026-07-25 (de emergência, após incidente de deploy — ver `TECHNICAL_DEBT.md` D8), schema confirmado objetivamente | — | — | Nenhuma |
| G06 | Despesas/Fornecedores | Documento de fornecedor com nº de lançamento + nº de documento distintos, emissão/vencimento/pagamento, valor/pago/saldo, **pagamentos parciais** | Pág. 4-6, 11 | ✅ em produção desde 2026-07-25 (Fase A.2 peça 1) | `documentoFornecedor` + `pagamentoDocumentoFornecedor` (`lib/db/schema.ts:556-633`), `lib/documentos-fornecedor.ts`, `app/actions/documentos-fornecedor.ts` — `valorPago`/`saldo` nunca persistidos, sempre calculados a partir da soma real dos pagamentos | Não | Baixo — migração puramente aditiva (`drizzle/0025_charming_epoch.sql`), aplicada em produção 2026-07-25, schema confirmado objetivamente (colunas/constraint verificadas por query direta) | — | — | Nenhuma |
| G07 | Despesas/Fornecedores | Anexos de fatura/recibo/contrato ao documento de fornecedor | Implícito (numeração formal de documentos sugere digitalização) | ✅ em produção desde 2026-07-25 (Fase A.2 peça 1) | `documentoFornecedor.anexoUrl`/`anexoNomeFicheiro` (`lib/db/schema.ts:582-583`), mesmo padrão de `seguro.anexoUrl` (Vercel Blob privado) | Não | Baixo | — | G06 | Nenhuma |
| G08 | Orçamento | Rubricas discriminadas com orçamentado vs. lançado vs. desvio vs. liquidado vs. por liquidar, **por categoria** | Pág. 9 | ✅ em produção desde 2026-07-25 (Fase A.2 peça 2) | `orcamentoRubrica` (`lib/db/schema.ts:399`), `getBalancoOrcamento` estendida (`app/actions/orcamentos.ts`) para cruzar orçamentado por rubrica com o real por categoria, incluindo categorias com despesa real mas sem rubrica associada | Não | Baixo — migração puramente aditiva (`drizzle/0026_great_sabretooth.sql`), aplicada em produção 2026-07-25, smoke test do diálogo "Gerir rubricas" em `/financas` (produção) sem erro | — | — | Nenhuma |
| G09 | Relatórios | Balanço contabilístico Ativo/Passivo/Situação Líquida | Pág. 10, com fórmulas | ✅ em produção desde 2026-07-25 (Fase B) — **atenção**: nome distinto de `/financas/balanco/[id]` (orçamento-vs-real), conforme já sinalizado na secção 6 | `getBalancoPatrimonial(exercicioId)` (`app/actions/contas-financeiras.ts`), separador "Balanço patrimonial" em `/financas` + página `/financas/balanco-patrimonial/[exercicioId]` | Não | Baixo — sem tabela nova, migração aditiva aplicada em produção 2026-07-25 (de emergência, ver `TECHNICAL_DEBT.md` D8); verificado com dados reais em produção (Ativo 7665,94 €, Passivo 0 €, Situação Líquida 7665,94 €); **não validado contra os totais reais da pág. 10 do PDF** (`Exemplo MBD.pdf` não está no repositório) | — | — | Validar contra o PDF de referência se e quando disponível |
...
| G17 | Assembleias | Anexos à ata | — | ✅ **corrigido 2026-08-24** (já listado, `FUNCTIONAL_GAPS.md:48`, P2) | Não encontrado | Sim, já confirmada | Baixo | P2 | Upload já maduro (Vercel Blob privado) | Campo `anexoUrl`/lista de anexos em `assembleia` | <br />**Correção 2026-08-24**: dado aqui como não implementado, mas os anexos à ata existem desde 2026-07-26 — tabela `assembleiaAnexo` (migração `0032`), com upload e eliminação enquanto a ata não estiver aprovada, em produção.
| G18 | Assembleias | Dossier automático de apoio à assembleia (capa, comunicação, índice, resumo, mapas, balanço, anexos, PDF único) | Documento inteiro é este dossier | ✅ **corrigido 2026-08-24** | Todas as peças existem em separado (relatório, mapa mensal, balanço orçamento, declaração de dívida) mas nunca consolidadas | Sim, confirmada — é a funcionalidade mais visível do PDF como um todo | Médio — tem base legal direta (art. 1432º "documentação de suporte" + art. 1436º/j "prestar contas") | P2 (Fase D, depois das peças financeiras que faltam) | G01, G08, G09 (para o dossier ser completo) | Ver secção 7 e Fase D no plano faseado | <br />**Correção 2026-08-24**: dado aqui como não implementado, mas o dossier de apoio à assembleia existe — `app/(app)/assembleias/[id]/dossier/page.tsx`, com resumo financeiro, mapa de saldos, exercícios e despesas para ratificar. Não foi verificado nesta passagem se cobre todas as peças que o PDF de referência inclui.
...
- Documentos de fornecedor com pagamentos parciais e anexo, rubricas orçamentais orçado vs. real (`documento_fornecedor`, `orcamento_rubrica` — Fase A.2, **em produção desde 2026-07-25**)
- Quotas extraordinárias ligadas a deliberação de assembleia, balanço patrimonial Ativo/Passivo/Situação Líquida (`movimento.assembleiaPontoId`, `getBalancoPatrimonial` — Fase B, **em produção desde 2026-07-25**)
...
**P1**: nenhuma lacuna P1 por resolver — todas implementadas (ver nota abaixo).

**P2**: G03 (% legal do FCR), G13 (conta corrente detalhada por fração), G17 (anexos à ata), G18 (dossier automático), G22 (testes de autorização HTTP).

G01, G02, G05, G06, G07, G08, G09 removidos destas listas — implementados e em produção (G01/G02 desde 2026-07-24, G06/G07/G08 desde 2026-07-25, G05/G09 desde 2026-07-25 — Fase B, aplicada de emergência após incidente de deploy, ver `TECHNICAL_DEBT.md` D8). Esta correção também resolve uma contradição interna pré-existente do documento: a matriz da secção 3 já marcava G01/G02 como ✅ em produção, mas continuavam listados aqui como lacunas P2.
...
### Fase A.1 — Exercícios e contas financeiras (primeiro incremento) — ✅ em produção desde 2026-07-24 (correção 2026-07-25: este cabeçalho ainda dizia "produção pendente", desatualizado — ver `ROADMAP.md`/`FUNCTIONAL_GAPS.md`, que já registavam a promoção)
...
### Fase A.2 — Documentos de fornecedor e rubricas orçamentais — ✅ em produção desde 2026-07-25
**Objetivo**: `documento_fornecedor` + `pagamento_documento_fornecedor` (peça 1), `orcamento_rubrica` (peça 2).
**Dependências**: Fase A.1 validada em produção — satisfeita (Fase A.1 promovida a produção em 2026-07-24, mesmo dia dos commits `cdf9007`/`93f605a` desta fase).
**Alterações de schema**: sim — migração `drizzle/0025_charming_epoch.sql` (peça 1: `documento_fornecedor`, `pagamento_documento_fornecedor`, `UNIQUE(id, condominioId)` em `movimento`) e `drizzle/0026_great_sabretooth.sql` (peça 2: `orcamento_rubrica`), ambas puramente aditivas. Mergeadas em `main` via PR #2 e #3, **aplicadas à base de dados de produção em 2026-07-25** seguindo `docs/PROCEDIMENTO_MIGRACAO_PRODUCAO.md` (snapshot manual antes de migrar, `drizzle-kit migrate`, `pnpm db:check-drift` confirmado "OK").
**Risco**: baixo, aditivo — mesmo padrão de risco da Fase A.1.
**Testes**: `lib/documentos-fornecedor.test.ts`, `lib/db/documentos-fornecedor.dbtest.ts` (incluindo a FK composta a rejeitar um pagamento associado a um documento de outro condomínio), `lib/db/balanco-orcamento.dbtest.ts` (rubrica dentro do orçado, acima do orçado, categoria sem rubrica). Em produção: schema confirmado objetivamente por query direta (colunas das 3 tabelas novas, constraint em `movimento`), `movimento` inalterado (34 linhas, antes e depois), smoke test de `/financas` (separador "Documentos de fornecedor" e diálogo "Gerir rubricas") sem erros e sem gravar dados de teste.
**Critério de conclusão**: cumprido — migrações aplicadas e verificadas sem regressões em produção 2026-07-25.
**Achado de processo (2026-07-25, sem impacto no schema)**: o hash de bookkeeping da migração `0025` em `drizzle.__drizzle_migrations` de desenvolvimento estava desalinhado do hash calculado a partir do ficheiro atual (hipótese mais provável: conversão de fim de linha CRLF/LF no Windows entre o momento em que foi aplicada a dev e agora) — corrigido com um `UPDATE` direto ao hash em dev (só bookkeeping, nenhuma tabela de negócio tocada), confirmado com `pnpm db:check-drift` a devolver "OK" depois.
...
**Alterações de schema**: `movimento.assembleiaPontoId` (G05) — migração `drizzle/0027_slippery_reavers.sql`, puramente aditiva (`ADD COLUMN` + FK + índice), aplicada em desenvolvimento e em produção, ambas em 2026-07-25.
**Estado: ✅ em produção desde 2026-07-25.**
**Risco**: baixo, aditivo — mas **materializou-se um risco de processo, não de schema**: o deploy automático da Vercel publicou o código em produção minutos depois do push para `main`, antes de a migração `0027` ter sido aplicada à BD de produção, derrubando todo o módulo `/financas` até a migração ser aplicada de emergência (ver `TECHNICAL_DEBT.md` D8, terceiro incidente deste tipo). O risco médio inicialmente previsto para o balanço patrimonial (G09) — validação contra o exemplo do PDF — não pôde ser eliminado por verificação externa: `Exemplo MBD.pdf` não está no repositório, pelo que os totais exatos da pág. 10 não foram confirmados. Mitigado por: fórmula Ativo = Passivo + Situação Líquida garantida por construção (resíduo), e reutilização de funções já testadas (`calcularSaldoConta`, `calcularSaldosDocumentosFornecedor`).
**Testes**: `lib/db/balanco-patrimonial.dbtest.ts` (cálculo correto dos 3 blocos com fixture própria, isolamento multi-tenant) e `lib/db/quota-extraordinaria.dbtest.ts` (elegibilidade de ponto de assembleia, 4 casos). Smoke test manual em runtime, em dev e em **produção real** (dados reais do condomínio piloto: balanço patrimonial com Ativo/Passivo/Situação Líquida corretos, diálogo de novo movimento com o seletor de deliberação, listagem de movimentos a carregar sem erro).
**Critério de conclusão**: cumprido tecnicamente (cálculo consistente, testado, e verificado com dados reais em produção); a comparação byte-a-byte com o PDF de referência fica pendente de o utilizador disponibilizar o documento, se ainda o tiver.
**Documentação afetada**: `FUNCTIONAL_GAPS.md`, `ROADMAP.md`, `TECHNICAL_DEBT.md` (D8, novo incidente).
