...
| G06 | Despesas/Fornecedores | Documento de fornecedor com nº de lançamento + nº de documento distintos, emissão/vencimento/pagamento, valor/pago/saldo, **pagamentos parciais** | Pág. 4-6, 11 | ✅ implementado em desenvolvimento (2026-07-24, Fase A.2 peça 1), produção pendente | `documentoFornecedor` + `pagamentoDocumentoFornecedor` (`lib/db/schema.ts:556-633`), `lib/documentos-fornecedor.ts`, `app/actions/documentos-fornecedor.ts` — `valorPago`/`saldo` nunca persistidos, sempre calculados a partir da soma real dos pagamentos | Não | Baixo — migração puramente aditiva (`drizzle/0025_charming_epoch.sql`), mergeada em `main` (commit `cdf9007`), ainda não aplicada à BD de produção | — | — | Aplicar migração `0025` à produção seguindo `docs/PROCEDIMENTO_MIGRACAO_PRODUCAO.md` |
| G07 | Despesas/Fornecedores | Anexos de fatura/recibo/contrato ao documento de fornecedor | Implícito (numeração formal de documentos sugere digitalização) | ✅ implementado em desenvolvimento (2026-07-24, Fase A.2 peça 1), produção pendente | `documentoFornecedor.anexoUrl`/`anexoNomeFicheiro` (`lib/db/schema.ts:582-583`), mesmo padrão de `seguro.anexoUrl` (Vercel Blob privado) | Não | Baixo | — | G06 | Aplicar migração `0025` à produção junto com G06 |
| G08 | Orçamento | Rubricas discriminadas com orçamentado vs. lançado vs. desvio vs. liquidado vs. por liquidar, **por categoria** | Pág. 9 | ✅ implementado em desenvolvimento (2026-07-24, Fase A.2 peça 2), produção pendente | `orcamentoRubrica` (`lib/db/schema.ts:399`), `getBalancoOrcamento` estendida (`app/actions/orcamentos.ts`) para cruzar orçamentado por rubrica com o real por categoria, incluindo categorias com despesa real mas sem rubrica associada | Não | Baixo — migração puramente aditiva (`drizzle/0026_great_sabretooth.sql`), mergeada em `main` (commit `93f605a`), ainda não aplicada à BD de produção | — | — | Aplicar migração `0026` à produção junto com G06/G07 |
...
- Documentos de fornecedor com pagamentos parciais e anexo, rubricas orçamentais orçado vs. real (`documento_fornecedor`, `orcamento_rubrica` — Fase A.2, **em desenvolvimento, produção pendente**)
...
G01, G02, G06, G07, G08 removidos destas listas — implementados (G01/G02 em produção desde 2026-07-24, G06/G07/G08 em desenvolvimento desde 2026-07-24, produção pendente). Esta correção também resolve uma contradição interna pré-existente do documento: a matriz da secção 3 já marcava G01/G02 como ✅ em produção, mas continuavam listados aqui como lacunas P2.
...
### Fase A.2 — Documentos de fornecedor e rubricas orçamentais — ✅ implementado em desenvolvimento (2026-07-24), produção pendente
...
**Alterações de schema**: sim — migração `drizzle/0025_charming_epoch.sql` (peça 1: `documento_fornecedor`, `pagamento_documento_fornecedor`, `UNIQUE(id, condominioId)` em `movimento`) e `drizzle/0026_great_sabretooth.sql` (peça 2: `orcamento_rubrica`), ambas puramente aditivas. Mergeadas em `main` via PR #2 e #3. **Nenhuma das duas foi ainda aplicada à base de dados de produção.**
**Risco**: baixo, aditivo — mesmo padrão de risco da Fase A.1.
**Testes**: `lib/documentos-fornecedor.test.ts`, `lib/db/documentos-fornecedor.dbtest.ts` (incluindo a FK composta a rejeitar um pagamento associado a um documento de outro condomínio), `lib/db/balanco-orcamento.dbtest.ts` (rubrica dentro do orçado, acima do orçado, categoria sem rubrica).
**Critério de conclusão**: falta aplicar as duas migrações à produção e validar sem regressões, seguindo `docs/PROCEDIMENTO_MIGRACAO_PRODUCAO.md`, antes de considerar a fase totalmente concluída.
