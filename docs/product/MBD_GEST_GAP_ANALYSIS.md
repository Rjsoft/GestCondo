...
| G05 | Receitas | Quotas extraordinárias distintas, ligadas a uma deliberação de assembleia | Pág. 3 ("Quota Extraordinária Obras", "Quota Extra - Rep. Fachadas") | ✅ implementado em desenvolvimento (2026-07-25, Fase B), produção pendente | `movimento.assembleiaPontoId` (FK simples, `lib/db/schema.ts`), `getPontosAprovadosParaQuota`/validação em `criarMovimento`/`atualizarMovimento` (`app/actions/financas.ts`) — só pontos aprovados de assembleias com ata aprovada são elegíveis | Não | Baixo — migração puramente aditiva (`drizzle/0027_slippery_reavers.sql`), aplicada em desenvolvimento, ainda não em produção | — | — | Aplicar migração `0027` à produção seguindo `docs/PROCEDIMENTO_MIGRACAO_PRODUCAO.md` |
...
| G09 | Relatórios | Balanço contabilístico Ativo/Passivo/Situação Líquida | Pág. 10, com fórmulas | ✅ implementado em desenvolvimento (2026-07-25, Fase B), produção pendente — **atenção**: nome distinto de `/financas/balanco/[id]` (orçamento-vs-real), conforme já sinalizado na secção 6 | `getBalancoPatrimonial(exercicioId)` (`app/actions/contas-financeiras.ts`), separador "Balanço patrimonial" em `/financas` + página `/financas/balanco-patrimonial/[exercicioId]` | Não | Baixo — sem tabela nova, migração aditiva já aplicada em dev; **não validado contra os totais reais da pág. 10 do PDF** (`Exemplo MBD.pdf` não está no repositório) — só a fórmula e a consistência interna (`lib/db/balanco-patrimonial.dbtest.ts`) foram verificadas | — | — | Aplicar migração `0027` à produção; validar contra o PDF de referência se e quando disponível |
...
- Quotas extraordinárias ligadas a deliberação de assembleia, balanço patrimonial Ativo/Passivo/Situação Líquida (`movimento.assembleiaPontoId`, `getBalancoPatrimonial` — Fase B, **em desenvolvimento, produção pendente**)
...
G01, G02, G06, G07, G08 removidos destas listas — implementados e em produção (G01/G02 desde 2026-07-24, G06/G07/G08 desde 2026-07-25). G05, G09 removidos — implementados em desenvolvimento desde 2026-07-25 (Fase B), produção pendente. Esta correção também resolve uma contradição interna pré-existente do documento: a matriz da secção 3 já marcava G01/G02 como ✅ em produção, mas continuavam listados aqui como lacunas P2.
...
**Alterações de schema**: `movimento.assembleiaPontoId` (G05) — migração `drizzle/0027_slippery_reavers.sql`, puramente aditiva (`ADD COLUMN` + FK + índice), aplicada em desenvolvimento 2026-07-25, produção pendente.
**Estado: ✅ implementado em desenvolvimento (2026-07-25), produção pendente.**
**Risco**: baixo, aditivo. O risco médio inicialmente previsto para o balanço patrimonial (G09) — validação contra o exemplo do PDF — não pôde ser eliminado por verificação externa: `Exemplo MBD.pdf` não está no repositório, pelo que os totais exatos da pág. 10 não foram confirmados. Mitigado por: fórmula Ativo = Passivo + Situação Líquida garantida por construção (resíduo), e reutilização de funções já testadas (`calcularSaldoConta`, `calcularSaldosDocumentosFornecedor`).
**Testes**: `lib/db/balanco-patrimonial.dbtest.ts` (cálculo correto dos 3 blocos com fixture própria, isolamento multi-tenant) e `lib/db/quota-extraordinaria.dbtest.ts` (elegibilidade de ponto de assembleia, 4 casos). Smoke test manual em runtime (dev): diálogo de novo movimento mostra o seletor de deliberação só em receitas, separador "Balanço patrimonial" mostra o estado vazio corretamente sem exercício configurado.
**Critério de conclusão**: cumprido tecnicamente (cálculo consistente e testado); a comparação byte-a-byte com o PDF de referência fica pendente de o utilizador disponibilizar o documento, se ainda o tiver.
**Documentação afetada**: `FUNCTIONAL_GAPS.md`, `ROADMAP.md`.
