...
Data: 2026-07-22, **atualizada 2026-07-24** (Fase A.1 — exercícios e contas financeiras), **2026-07-25** (identidade legal do operador decidida — item 1; foro competente confirmado — item 2; modelo de Contrato SaaS criado — item 5; processo mínimo de oposição/limitação definido — item 7; progresso no item 6 — DPAs de subcontratantes localizados, DPA da Neon confirmado sem restrição de plano; progresso no item 3 — jurisprudência do STJ encontrada, questão continua em aberto). Consolida, num só sítio, o que já estava disperso por `RGPD_AUDIT.md`, `TERMS_OF_USE_REVIEW.md`, `PRIVACY_POLICY_REVIEW.md`, `LEGAL_COMPLIANCE_AUDIT.md` e `ROADMAP.md`. "Cliente externo" = alguém que não sejas tu: um condómino real de fora, ou uma empresa de administração terceira.
...
| 3 | 🟡 Classificação B2B / B2C / misto nos Termos | Determina se se aplicam regras de proteção do consumidor (RAL/ODR já estão genéricas, mas a classificação em si falta) | **Progresso 2026-07-25** (ver `docs/legal/TERMS_OF_USE_REVIEW.md` secção 6): encontrada jurisprudência real do STJ (2022, 2023) a tratar o condomínio como consumidor quando as frações são total ou maioritariamente residenciais — contraria a leitura preliminar anterior ("tende para B2B"). Questão fica genuinamente em aberto, mais fundamentada, **ainda não decidida**. Continua a depender de confirmação jurídica externa |
...
| 6 | 🟡 DPA dos teus próprios subcontratantes (Neon, Resend, Vercel) | Accountability RGPD (art. 28º) — precisas de conseguir demonstrar que também tu tens isto em ordem com quem processa dados por tua conta | **Progresso 2026-07-25** (ver `docs/legal/DATA_SUBPROCESSORS_REGISTER.md`): Resend — DPA aplica-se automaticamente, sem ação adicional. Neon — DPA aplica-se automaticamente via Termos de Serviço, sem restrição de plano confirmada (fonte oficial da Neon), sem ação adicional. Vercel — **DPA só se aplica em planos Pro/Enterprise; o projeto está no Hobby, logo ainda não se aplica**. Upgrade a Pro necessário, mas **adiado por decisão do utilizador até ao primeiro contacto com cliente externo** — único ponto do item 6 que continua efetivamente em aberto |
...
**Achado 2026-07-25**: o plano Hobby da Vercel restringe o uso a "non-commercial, personal use only" (termos oficiais da Vercel) — incompatível com faturar clientes via RJCSI, independentemente da escala. Upgrade a Vercel Pro (~20-24 €/mês) resolve isto e desbloqueia automaticamente o DPA da Vercel (item 6). Não bloqueia continuação do desenvolvimento nem a fase atual sem clientes — bloqueia sim disponibilização comercial real.
