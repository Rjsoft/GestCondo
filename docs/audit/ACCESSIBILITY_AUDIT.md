...
- `nova-conta-financeira-dialog.tsx` (o cenário mais denso, com "Opções avançadas" expandidas) limitado individualmente a `max-h-[calc(100dvh-2rem)] overflow-y-auto` no commit `66a4182`, para impedir que o conteúdo exceda a altura visível do ecrã e que os botões fiquem inacessíveis.
- **Atualizado 2026-07-25**: o mesmo problema (conteúdo mais alto do que o ecrã, sem forma de lá chegar) foi confirmado nos **restantes 33 diálogos**, que não tinham a correção individual acima — reparado pelo próprio utilizador ao ver o diálogo "Novo movimento" cortado num ecrã pequeno. Corrigido de forma global em `components/ui/dialog.tsx` (`DialogContent`: `max-h-[85vh] overflow-y-auto`), em vez de repetir a correção dialog a dialog — cobre agora todos os 34 diálogos da aplicação, incluindo os 3 já corrigidos individualmente (a regra local mais específica desses 3 continua a prevalecer via `tailwind-merge`, sem conflito). Confirmado visualmente em runtime (dev): "Novo movimento" mostra barra de scroll vertical em vez de cortar o botão "Guardar".
- **Achado adicional (2026-07-25, não corrigido — fora do pedido)**: o padrão "Opções avançadas" de `nova-conta-financeira-dialog.tsx` (botão simples + `useState`, sem `aria-expanded`/`aria-controls`) é **diferente e menos acessível** do que o novo componente partilhado `components/ui/collapsible.tsx` (criado 2026-07-25, envolve `@base-ui/react/collapsible`, que já gere `aria-expanded` e a associação semântica trigger↔painel automaticamente). Usado para simplificar 4 diálogos (`novo-movimento-dialog.tsx`, `editar-movimento-dialog.tsx`, `nova-fracao-dialog.tsx`, `editar-fracao-dialog.tsx`, `novo-seguro-dialog.tsx`) com a técnica "Mais opções" (campos secundários escondidos por omissão). Ficam agora **dois padrões distintos** de disclosure no código — vale a pena, numa sessão futura, migrar `nova-conta-financeira-dialog.tsx`/`editar-conta-financeira-dialog.tsx`/`novo-exercicio-dialog.tsx` para o novo componente partilhado, por consistência e por ser mais acessível.
...
## 6. Primeiro teste real com o NVDA (2026-07-29) e verificação técnica de seguimento (2026-07-30)

**2026-07-29**: primeiro teste real com o NVDA, feito por um utilizador cego externo ao desenvolvimento, seguindo `docs/GUIA_TESTE_NVDA.md` — o achado A4/L4 (ausência de teste real) deixa de estar em aberto. Não ficou um registo escrito problema-a-problema (o utilizador reportou a impressão geral, não uma lista formal); o resultado relatado foi que a navegação **exclusivamente por teclado, sem rato, é difícil** em geral. Sem lista discriminada, não é possível cruzar cada dificuldade sentida com uma causa técnica específica — o que se segue é uma verificação técnica de seguimento, não uma confirmação direta das causas exatas sentidas pelo utilizador.

**2026-07-30**: verificação técnica dirigida à navegação só por teclado (sem leitor de ecrã a correr — inspeção do `document.activeElement` real via DevTools a cada tecla, não simulação nem suposição), cobrindo os padrões de interface usados em toda a aplicação (não só um módulo). Método: navegação real por `Tab`/`Shift+Tab`/setas/`Enter`/`Escape`, com o foco efetivamente resultante confirmado no DOM a cada passo — não inferido de capturas de ecrã.

### 6.1 Achados corrigidos — A5 e A6

| # | Achado | Gravidade | Detalhe | Correção |
|---|---|---|---|---|
| A5 | Botões que navegam para outra página (renderizados como `<Link>` via `render={<Link .../>}`) sem `nativeButton={false}` | Alta (no caso da paginação) / Baixa (nos restantes) | Confirmado por leitura do código-fonte do `@base-ui/react` (`useButton.js`, `useFocusableWhenDisabled.js`), não só pelo aviso na consola: com `nativeButton` a assumir `true` (omissão) sobre um elemento que na realidade é um `<a>`, o atributo `disabled` é escrito no DOM sem efeito (`<a>` não suporta `disabled`), **sem `aria-disabled`** e **sem remoção da ordem de tabulação** — nos botões "Anterior"/"Seguinte" da paginação (`components/ui/pagination-controls.tsx`), isto significa que um botão "desativado" (ex. "Anterior" na página 1) continua totalmente alcançável por `Tab`, o leitor de ecrã não anuncia que está desativado, e o clique é bloqueado sem qualquer explicação — exatamente o tipo de confusão silenciosa que dificulta a navegação só por teclado. Nos restantes 14 casos (botões de ação como "Ver recibo", "Declaração", "Convocatória (PDF)", que nunca ficam `disabled`), o impacto real é menor (inconsistência de `role`/comportamento de tecla `Space` face aos outros botões), mas o aviso da biblioteca aplicava-se a todos. | **Corrigido** — `nativeButton={false}` acrescentado nos 16 pontos (`grep -rn "render={<Link" components app` menos os 2 casos em `DropdownMenuItem`, que já são não-nativos por omissão nesse componente e não precisavam de alteração). Confirmado por leitura do código do Base UI que isto passa a atribuir `tabIndex=-1` e `aria-disabled="true"` corretamente quando `disabled`. Verificado: `tsc --noEmit`, `eslint`, `vitest run` (134/134) sem regressões. |
| A6 | Foco não gerido nas navegações internas (client-side, `next/link`) entre páginas — **o achado mais grave desta sessão, provável causa principal da dificuldade relatada em 2026-07-29** | Alta | Descoberto ao reproduzir literalmente o percurso descrito pelo próprio utilizador ("estou na barra lateral em Finanças, carrego Enter — como mudo de separador?"). Confirmado passo a passo com `document.activeElement`: ao clicar num link da barra lateral (ex. "Finanças"), o Next.js faz uma transição interna sem recarregar a página — e o foco **fica preso no próprio link clicado**, em vez de ir para o conteúdo novo. O link "Saltar para o conteúdo" (A1) só resolve isto numa primeira visita à app (carregamento completo da página); em todas as navegações seguintes pela barra lateral, fica *atrás* do ponto onde o utilizador está, inacessível sem retroceder. Medido concretamente: de "Finanças" até ao primeiro separador ("Movimentos") eram precisos **14 `Tab`** — o resto da barra lateral (Avisos, Assembleias, Ocorrências, Documentos, Mensagens, Fornecedores, Frações, Condóminos, Auditoria, Os meus dados, Ajuda, Condomínio, Terminar sessão) tinha de ser percorrido por inteiro antes de chegar ao conteúdo. | **Corrigido** — `components/app-shell.tsx`: nova `useRef` no `<main id="main-content">` e um `useEffect` sobre `usePathname()` que move o foco para o conteúdo principal a cada mudança de página (exceto na primeira renderização, para não roubar o foco ao carregar a app pela primeira vez — o `Tab` inicial continua a ir para o link "Saltar para o conteúdo", confirmado sem alteração). Não dispara em mudanças de `searchParams` na mesma página (ex. paginação), só em mudança de rota — confirmado que paginar uma tabela não força o foco de volta ao topo. Medido depois da correção: de "Finanças" até "Movimentos" passou a **1 `Tab`**. Verificado: `tsc --noEmit`, `eslint`, `vitest run` (134/134), `next build`, sem regressões. |

### 6.2 Padrões verificados e confirmados corretos (não é preciso corrigir)

Verificado com o mesmo método (foco real no DOM, não suposição), um por um:

| Padrão | Onde | Verificação | Resultado |
|---|---|---|---|
| Link "saltar para o conteúdo" | `app-shell.tsx` | Primeiro `Tab` da página, num carregamento completo (não numa navegação interna — ver A6) | Confirmado — primeiro elemento focável, `href="#main-content"` |
| Ordem de tabulação do menu lateral | `app-shell.tsx` | Sequência de `Tab` item a item | Linear, sem saltos nem armadilhas |
| Separadores (`Tabs`, ex. em Finanças) | `components/financas/financas-tabs.tsx` | `ArrowRight`/`ArrowLeft` com foco num separador | Muda de separador e ativa automaticamente — padrão WAI-ARIA "tabs" correto; evita ter de percorrer os 10 separadores um a um só com `Tab` |
| Menu suspenso (`Select`, ex. critério de rateio) | `components/ui/select.tsx` | `Enter` abre com foco na opção atual; setas movem; `Enter` seleciona; `Escape` fecha sem alterar e devolve o foco ao campo | Todos os passos corretos |
| Diálogos (`Dialog`, ex. "Nova fração") | `components/ui/dialog.tsx` | Foco inicial no 1º campo; `Tab` percorre todos os campos e botões; ordem correta; ao chegar ao fim, volta ao 1º campo (contenção fechada); `Escape`/"Fechar" devolvem o foco ao botão que abriu | Correto |
| Menu de ações (`DropdownMenu`, ex. "Ações" numa linha de tabela) | `components/ui/dropdown-menu.tsx` | `Enter` abre com foco no 1º item; setas movem; `Escape` fecha e devolve o foco ao botão "Ações" | Correto |
| `Checkbox` | `components/ui/checkbox.tsx` | Verificado por leitura do código (usa `@base-ui/react/checkbox` sem lógica de teclado manual sobreposta) | Sem padrão de risco identificado; não repetido em runtime por já ser o mesmo primitivo confirmado nos outros componentes |
| Campos de carregamento de ficheiros (`type="file"`) | 10 diálogos | `<input type="file">` nativo em todos, sem *wrapper* customizado | Focável e ativável por teclado por comportamento nativo do browser, sem risco |

### 6.3 Observação menor, não corrigida (baixa gravidade, sem impacto confirmado no utilizador)

Ao pressionar `Tab` muito rapidamente (rajada, sem pausa) logo a seguir a abrir um diálogo pela primeira vez numa página recém-carregada, o foco esteve, por um instante, num elemento sem nome acessível (`<span>` vazio), antes de se autocorrigir sozinho para dentro do diálogo cerca de meio segundo depois — reproduzido 2 de 6 tentativas, sempre a autocorrigir-se, nunca a ficar preso fora do diálogo. Parece uma janela curta durante a animação de abertura/hidratação inicial da página, não uma falha da armadilha de foco em si (que, em todas as tentativas com ritmo normal ou com o diálogo reaberto na mesma sessão, funcionou sem falhas). **Não corrigido nesta sessão** — não há uma causa raiz confirmada (só a hipótese de temporização), e mexer na lógica interna de foco da biblioteca sem essa certeza arrisca introduzir uma regressão pior do que o problema. Fica registado para investigação futura se voltar a ser reportado por um utilizador real.

### 6.4 Conclusão desta verificação

Esta verificação confirma tecnicamente **dois** problemas reais e concretos (A5 e A6, ambos corrigidos) e não encontra falhas nos restantes padrões de interface interativa usados transversalmente na aplicação (separadores, menus, diálogos, campos de escolha, checkboxes, upload). O A6 (foco preso no link clicado a cada navegação interna pela barra lateral, obrigando a percorrer o resto do menu por inteiro antes de chegar ao conteúdo) é, das duas, a explicação técnica mais plausível para a dificuldade genérica relatada em 2026-07-29 — afeta **todas** as páginas, em **todas** as navegações a partir da segunda, não só um caso específico como o A5. Só foi descoberto ao reproduzir literalmente o percurso que o próprio utilizador descreveu ao usar a app ("estou na barra lateral em Finanças... como mudo de separador?") — a verificação inicial desta sessão, feita por navegações completas (`navigate()` de URL, não cliques internos), não o teria apanhado, porque uma navegação completa reinicia sempre o foco corretamente; só uma transição interna real do Next.js expõe o problema.

Isto **não** esgota nem substitui L4/L5 — o teste do dia 2026-07-29 já cumpriu o requisito de "teste real com leitor de ecrã", mas sem lista discriminada não se sabe se a dificuldade sentida pelo utilizador tinha outras causas para além de A5/A6 (ex. verbosidade dos anúncios do NVDA, densidade de informação em tabelas grandes, desconhecimento de atalhos como as setas nos separadores — ver `docs/GUIA_NVDA_NARRADOR.md`, escrito nesta sessão precisamente para cobrir essa lacuna de conhecimento). **Recomendação**: repetir o teste com o mesmo utilizador (ou outro, para variedade de configuração), agora que A5 e A6 estão corrigidos e existe um manual dedicado — só esse segundo teste real confirma se a dificuldade relatada foi resolvida. Uma eventual auditoria independente adicional deve ter em conta especificamente este tipo de achado — problemas que só aparecem ao seguir um percurso real de navegação encadeado, não numa verificação isolada por página.

### 6.5 Auditoria independente (2026-07-30) — A7, A8, A9

Pedido explícito do utilizador: uma segunda auditoria, feita por um agente sem conhecimento prévio dos achados A1–A6 (para evitar viés de confirmação), com instrução explícita de só comparar com este documento **depois** de ter a sua própria lista. Ambiente próprio (conta e condomínio fictícios distintos), mesmo método (`document.activeElement` real, não suposição). Confirmou A5 e A6 de forma independente, sem discrepância, e todos os padrões da secção 6.2. Encontrou três achados novos, todos corrigidos e reverificados no browser real após a correção:

| # | Achado | Gravidade | Detalhe | Correção |
|---|---|---|---|---|
| A7 | Transições de ecrã por estado do componente (sem mudança de rota) sem gestão de foco | Média/Alta | `/sign-up` depois de submeter ("Falta só um passo") e `/esqueci-password` depois de submeter ("Verifique o seu email") — são trocas de conteúdo por `useState` interno, não mudanças de URL, por isso a correção A6 (que só atua sobre `usePathname()`) não as cobre. Confirmado: `document.activeElement` = `BODY` depois da transição, sem região `aria-live` a compensar. | **Corrigido** — `components/auth-form.tsx` e `components/esqueci-password-form.tsx`: `useRef`+`tabIndex={-1}` no título (`h1`) do ecrã de sucesso, com `useEffect` a focá-lo quando o estado muda para `true`. Reverificado no browser: foco corretamente em `H1` nos dois fluxos. |
| A8 | Botões de paginação desativados sem indicação visual | Baixa | Extensão do A5, já corrigido no essencial (teclado/clique). As classes `disabled:opacity-50 disabled:pointer-events-none` do Tailwind usam o pseudo-seletor `:disabled`, que só se aplica a elementos nativos de formulário — nunca a um `<a>` (o botão é renderizado como `Link`). Confirmado por `getComputedStyle`: `opacity: 1`, `pointer-events: auto` num botão já com `aria-disabled="true"` correto. | **Corrigido** — `components/ui/button.tsx`: acrescentadas as variantes `aria-disabled:pointer-events-none aria-disabled:opacity-50` (Tailwind v4 já usa este padrão no mesmo ficheiro para `aria-invalid`). Reverificado com 21 avisos de teste a forçar paginação real: `opacity: 0.5`, `pointer-events: none`, confirmado no DOM. |
