...
| A2 | Contraste de cor não verificado com ferramenta dedicada | Não verificado | Os tokens de cor (`app/globals.css`) usam OKLCH; `--muted-foreground` (L≈0.52) sobre `--background` (L≈0.985) parece ter margem suficiente para texto grande, mas não posso confirmar o rácio exato (WCAG AA exige 4.5:1 para texto normal, 3:1 para texto grande) sem uma ferramenta de contraste real — não vou reportar isto como "passa" sem verificar | Correr o Chrome DevTools (Lighthouse → Accessibility) ou a extensão axe DevTools sobre 2-3 páginas representativas; ajustar `--muted-foreground` se necessário |
...
2. **Verificação com ferramenta automática**: A2, correr Lighthouse/axe sobre as páginas principais (painel, finanças, fornecedores, ocorrências) para apanhar problemas de contraste e ARIA que a revisão manual de código não deteta.
3. **Teste real com o NVDA**: A3, A4 — percorrer os fluxos principais (login, criar um movimento, abrir um diálogo, navegar pela tabela de finanças) com o NVDA a correr, idealmente feito pelo utilizador ou por alguém que o utilize no dia a dia — é o único teste que conta como prova real de que a app "funciona com o NVDA".
