...
|---|---|---|---|
| Link "saltar para o conteúdo" | `app-shell.tsx` | Primeiro `Tab` da página | Confirmado — primeiro elemento focável, `href="#main-content"` |
...
Esta verificação confirma tecnicamente **um** problema real e concreto (A5, corrigido) e não encontra falhas nos restantes padrões de interface interativa usados transversalmente na aplicação (separadores, menus, diálogos, campos de escolha, checkboxes, upload). Isto **não** esgota nem substitui L4/L5 — o teste do dia 2026-07-29 já cumpriu o requisito de "teste real com leitor de ecrã", mas sem lista discriminada não se sabe se a dificuldade sentida pelo utilizador tinha outras causas para além da A5 (ex. verbosidade dos anúncios do NVDA, densidade de informação em tabelas grandes, desconhecimento de atalhos como as setas nos separadores — ver `docs/GUIA_NVDA_NARRADOR.md`, escrito nesta sessão precisamente para cobrir essa lacuna de conhecimento). **Recomendação**: repetir o teste com o mesmo utilizador (ou outro, para variedade de configuração), agora que a A5 está corrigida e existe um manual dedicado — só esse segundo teste real confirma se a dificuldade relatada foi resolvida.
