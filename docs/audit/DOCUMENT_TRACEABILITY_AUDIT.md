...
| Conciliação bancária | ✅ Fase A.1, **resolvido em desenvolvimento (commit `8ac97f6`)** — `conciliarLinha()` regista o identificador do movimento, valida a compatibilidade da conta quando ambas estão definidas, e passa a incluir no texto `— conta financeira ID N` (só o ID já disponível, sem query adicional, nunca IBAN/banco/saldo) |
| Desconciliação bancária | ✅ Fase A.1, **resolvido em desenvolvimento (commit `8ac97f6`)** — o texto passa a incluir `— conta financeira ID N`; a leitura do movimento envolvido passou também a filtrar por `condominioId`, não só pelo id sequencial |
...
| T3 | Conta financeira ausente do texto de conciliação — **resolvida em desenvolvimento, commit `8ac97f6`** | Baixa | Exigia consulta cruzada ao movimento/extrato para saber a que conta uma conciliação pertence. **Resolvido**: `conciliarLinha`/`desfazerConciliacao` passam a incluir `— conta financeira ID N` no texto, usando só o ID já disponível em memória (`linha.contaFinanceiraId ?? mov.contaFinanceiraId`), sem query adicional; nunca inclui IBAN, banco, saldo ou titular; registos antigos sem conta associada mantêm o texto anterior, sem sufixo. Corrigida no mesmo diff a leitura do movimento em `desfazerConciliacao`, que só filtrava por `movimento.id` — passou a filtrar também por `movimento.condominioId` (isolamento multi-tenant). 8 testes de integração novos (`lib/db/extrato-conciliacao.dbtest.ts`); total de integração passou para 31/31 | `conciliarLinha`/`desfazerConciliacao` (`app/actions/extrato.ts`) | Concluída em desenvolvimento; validar novamente após eventual promoção a produção | Feito | P3 |
