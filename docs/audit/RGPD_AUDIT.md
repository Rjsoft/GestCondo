...
| RGPD-04 | ~~Só `movimento` usava soft-delete~~ **Resolvido 2026-07-22** — `seguro`, `aviso`, `documento`, `ocorrencia` passaram todos a soft-delete; `fracao`/`membro` continuam com `DELETE` físico (exceto autoeliminação de conta, já auditada) | Média | — | Art. 5º/1/e RGPD | — | `lib/db/schema.ts` |
