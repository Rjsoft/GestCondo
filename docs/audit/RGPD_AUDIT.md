...
| RGPD-02 | ~~Exportação de dados pessoais incompleta~~ **Resolvido 2026-07-22** — agora inclui ocorrências reportadas, presenças/votos da fração e (se o perfil tiver acesso financeiro) movimentos da fração; passou também a ficar registada em `audit_log` (ver AUDIT-02) | Média | — | Arts. 15º/20º RGPD | — | `app/actions/perfil.ts` |
...
| RGPD-04 | Só `movimento` e (desde 2026-07-22) `seguro` usam soft-delete; `fracao`/`aviso`/`documento`/`ocorrencia` continuam com `DELETE` físico | Média | P2 | Art. 5º/1/e RGPD | ~~Avaliar necessidade de soft-delete adicional, especialmente `seguro`~~ — feito para `seguro`; falta o resto | `lib/db/schema.ts` |
