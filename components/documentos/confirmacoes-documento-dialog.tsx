...
import { getConfirmacoesLeituraDocumento } from '@/app/actions/documentos'
...
export function ConfirmacoesDocumentoDialog({ documentoId, total }: { documentoId: number; total: number }) {
      ...
      onOpenChange={(open) => {
        if (open && !lista) {
          startTransition(async () => {
            setLista(await getConfirmacoesLeituraDocumento(documentoId))
          })
        }
      }}
    >
