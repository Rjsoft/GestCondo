...
| Frações autónomas | 🟡 Parcial | `app/(app)/fracoes/page.tsx` + schema `fracao`: identificação, proprietário (texto livre), permilagem, contactos, notas. **2026-07-23**: campos `letra` (código curto, ex. "A", alinhado com a notação usada nos mapas de administrações externas como a MBD Gest) e `tipoTitular` (proprietário/inquilino/usufrutuário/locatário/antigo), ambos opcionais, editáveis em `EditarFracaoDialog`. Sem condomínio associado. | P1 (completar) |
