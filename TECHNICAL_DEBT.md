...
- **Base de dados:** PostgreSQL (Neon) via `pg` + Drizzle ORM 0.45, com `drizzle-kit` e migrações versionadas em `drizzle/` desde 2026-07-06 (ver T4). São 25 migrações aplicadas a 2026-07-24, em duas bases de dados distintas (branches `development` e `production`) — a aplicação a produção continua a ser manual, ver D8.
