...
| T3 | ~~Sem testes de nenhum tipo~~ **Parcialmente resolvido 2026-07-06** | Alta | `vitest` configurado (`vitest.config.ts`, script `pnpm test`). 38 testes unitários cobrindo `lib/perfis.ts` (toda a matriz de permissões: os 6 perfis × as 4 funções de autorização, mais o bypass de super admin) e `lib/format.ts`. **Continua a faltar**: testes de integração/e2e contra uma base de dados real (autorização ponta-a-ponta nas server actions, isolamento multi-tenant, fluxo de aprovação) — não é possível escrevê-los com confiança neste ambiente sem Postgres disponível; ver `MVP_PLAN.md`. | Assim que houver uma BD de desenvolvimento (ver `workflow` local com Neon), prioridade nº1 é um teste de isolamento multi-tenant. |
