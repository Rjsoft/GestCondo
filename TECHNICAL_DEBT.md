...
| D5 | ~~Sem soft-delete em nenhuma tabela~~ **Parcialmente resolvido 2026-07-06** | Alta | `movimento.deletedAt` implementado (ver `SECURITY_AUDIT.md` S17) — o caso mais crítico (retenção contabilística/fiscal). `fracao`, `aviso`, `documento`, `ocorrencia`, `membro` continuam a usar `DELETE` físico, agora pelo menos rastreado no `audit_log`. |
...
| D8 | Sem passo automático de migração no deploy de produção | Alta | Causou um incidente real em 2026-07-21 (ver `ROADMAP.md` Fase 2 item 7): a migração `0012` ficou aplicada só à BD de desenvolvimento (Neon, branch `development`) e nunca à de produção (branch `production`), derrubando `/financas` por inteiro em produção durante horas até ser diagnosticado (erro 500, tabela em falta). O projeto tem duas bases de dados Neon distintas (branches `development`/`production`) mas nenhum passo de CI/CD corre `drizzle-kit migrate` contra produção — depende inteiramente de o humano se lembrar de o fazer manualmente depois de cada deploy que altere o schema. Agravado por: o `DATABASE_URL` de produção está marcado "Sensitive" na Vercel, o que impede `vercel env pull` de o obter — só é possível migrar com a connection string obtida diretamente no Neon. | Adicionar um passo de `pnpm db:migrate` (contra produção) ao workflow de deploy — ou, no mínimo, um lembrete/checklist manual associado a qualquer PR que inclua ficheiros novos em `drizzle/`. |
