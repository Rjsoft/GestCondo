...
| S15 | `typescript.ignoreBuildErrors: true` | Média | O build de produção ignora erros de tipo. Já existem hoje 13 erros de tipo reais (incompatibilidade `@base-ui/react` — ver `TECHNICAL_DEBT.md`) escondidos por esta flag. Isto significa que **qualquer** erro de tipo futuro, incluindo um que introduza um bug de segurança (ex. comparação de tipo errada numa verificação de permissão), passa despercebido no build. | Remover a flag assim que os erros de tipo pré-existentes forem corrigidos (upgrade/ajuste do `@base-ui/react`), e nunca reativar. |
