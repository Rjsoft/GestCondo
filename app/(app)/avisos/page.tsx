...
export default async function AvisosPage({
  ...
  const { avisos, totalPages } = await getAvisos({ page, search })
  ...
  return (
    <div>
      ...
      <div className="mb-4">
        <SearchInput placeholder="Pesquisar avisos..." />
      </div>
      ...
      {avisos.length === 0 ? (
        <Card>
          <CardContent className="flex flex-col items-center gap-2 py-16 text-center text-muted-foreground">
            ...
            <p>{search ? 'Nenhum aviso encontrado.' : 'Ainda não existem avisos.'}</p>
          </CardContent>
        </Card>
      ) : (
        <>
          ...
          <PaginationControls
            ...
            buildHref={(p) => `/avisos?${new URLSearchParams({ ...(search ? { q: search } : {}), page: String(p) }).toString()}`}
          />
        </>
      )}
    </div>
