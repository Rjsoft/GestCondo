...
| Métricas de utilização (páginas visitadas, referrer, país aproximado) | Vercel Analytics (`app/layout.tsx`, só em produção) | Todos os visitantes | Métricas agregadas de utilização | **Identificado 2026-07-22.** Segundo o fornecedor, sem cookies nem armazenamento de IP em claro — não confirmado formalmente pelo operador. Não mencionado até agora na Política de Privacidade nem no `RAT.md` (já corrigido no `RAT.md`; **correção 2026-08-24**: a Política de Privacidade existe desde 2026-07-09, `app/privacidade/page.tsx` — o que falta é confirmar que menciona este tratamento em concreto). |
| IBAN associado a conta financeira | `conta_financeira.iban` | Normalmente o condomínio (pessoa coletiva); pode constituir dado pessoal quando a conta pertence ou permite identificar uma pessoa singular (ex. conta pessoal de um administrador residente, usada excecionalmente) | Gestão das contas bancárias e de caixa do condomínio (Fase A.1, **em produção desde 2026-07-24**) | Normalizado e validado por checksum (mod-97) em `lib/contas-financeiras.ts`; o valor completo só é devolvido a perfis com `temConsultaGestao()` (`getContasFinanceiras`/`getSaldosContas` mascaram para os restantes); não é registado integralmente em `audit_log`. Ver `SECURITY_AUDIT.md` S19, `RAT.md` secção 3, `docs/legal/DATA_RETENTION_MATRIX.md`. |

**Nota sobre titularidade e IBAN**: o tratamento RGPD depende da titularidade da conta. Quando a conta é titulada pelo condomínio enquanto pessoa coletiva, o IBAN não é automaticamente um dado pessoal. Quando a conta pertence a uma pessoa singular ou permite identificá-la, deve ser tratado como dado pessoal financeiro e sujeito aos direitos e controlos aplicáveis (ver secção 3). **Pendente como decisão futura**: representar de forma inequívoca a titularidade da conta no modelo de dados, caso se confirme a necessidade de contas pessoais ou transitórias — sem criar campo ou migração nova nesta revisão documental.

**Não recolhido hoje, mas necessário para o produto-alvo** (a avaliar cuidadosamente quando implementado): NIF de proprietários (para recibos/declarações de dívida), morada de correspondência se diferente da fração, dados de inquilinos distintos dos proprietários, documentos de identificação, fotos anexadas a ocorrências (podem conter pessoas identificáveis).

## 2. Base legal e minimização

- **Documentação de base legal — corrigido 2026-08-24.** Esta linha dizia que "não existe qualquer documentação de base legal por tipo de tratamento": é falso desde 2026-07-09, o `RAT.md` documenta a base legal em cada um dos seis tratamentos registados. O que continua por fazer é a validação jurídica dessas bases por um jurista, e a sua reprodução nos textos voltados ao titular. Para a generalidade dos dados de gestão de condomínio (quotas, frações, contactos do administrador), a base legal previsível é **execução de contrato/obrigação legal** (regime da propriedade horizontal, Código Civil arts. 1430º e seguintes), não consentimento — o que é bom (não depende de opt-in revogável), mas **tem de ser documentado formalmente**, nomeadamente num Registo de Atividades de Tratamento (art. 30º RGPD).
...
- **Atualização 2026-07-24 — Fase A.1, desenvolvimento**: `exercicio_financeiro`, `conta_financeira` e `saldo_inicial_conta` não dispõem, nesta fase, de operação funcional de eliminação física ou lógica. As contas são encerradas, os exercícios são fechados ou reabertos e as correções de saldo inicial são auditadas, preservando o valor anterior e o novo valor. **Este comportamento corresponde ao estado técnico atual e não substitui uma política formal de retenção.** Os prazos e critérios de conservação constam de `docs/legal/DATA_RETENTION_MATRIX.md` e permanecem sujeitos à validação jurídica e contabilística indicada nesse documento — não existe eliminação automática enquanto essa validação não estiver concluída. Confirmado: contas encerradas permanecem no histórico; exercícios fechados permanecem no histórico; alterações críticas (fecho, reabertura, correção de saldo) são auditadas; não existe apagamento silencioso de saldos; o `audit_log` não contém o IBAN completo.
...
- [x] Isolamento multi-condomínio implementado — 2026-07-06 (schema + todas as queries; falta o fluxo de onboarding para um 2º condomínio, ver `FUNCTIONAL_GAPS.md`). **2026-07-24 — desenvolvimento**: o isolamento multi-condomínio foi alargado às entidades da Fase A.1 através de relações compostas com `condominioId`, complementadas por validações nas Server Actions e testes de integração. A migração e a validação em produção continuam pendentes.
- [x] IBAN mascarado por permissão — **2026-07-24 — desenvolvimento**: o IBAN completo é restringido por permissão, mascarado quando apresentado sem necessidade de detalhe e não é registado integralmente no `audit_log` (ver `SECURITY_AUDIT.md` S19). Mantém-se pendente a validação após futura promoção a produção.
